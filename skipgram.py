...
class SkipGram:
    ...
    def forward(self, index1, index2, W, W_prime):
        w_c = W[index1]
        w_prime_c = W_prime[index2]
        z = np.dot(w_prime_c, w_c)
        return z
        """
        Implement forward!
    ...
    def optimize(self, learning_rate, gradients, W, W_prime):
        ...
        for grad, center_i, context_i in gradients:
            self.W_prime[context_i] += grad * W[center_i] * learning_rate
        # Update W with W_prime_original and grad.
        for grad, center_i, context_i in gradients:
            self.W[center_i] += grad * W_prime_original[context_i] * learning_rate
    ...
    def subsampling(self, sample_bound, sentence):
        ...
        """
        def calculate_prob(f, t):
            return 1 - t/f - np.sqrt(t/f)
        sampled_word_indices = []
        for word in sentence.strip().split(' '):
            if self.vocab.word2count.get(word) is None:
                continue
            else:
                word_frequency = self.vocab.word2count[word]/self.vocab.total_words
                prob = calculate_prob(word_frequency, sample_bound)
